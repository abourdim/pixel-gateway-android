#!/usr/bin/env bash
# ============================================================================
#  Pixel Gateway Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/app-02-pixel-gateway}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.pixelgateway}"
AVD_NAME="${AVD_NAME:-pixel_gateway_test}"
...
cmd_test() {
  ...
  local out="/tmp/pixel-gateway-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Pixel Gateway Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
